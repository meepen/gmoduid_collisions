...
void CRC32_ProcessBuffer(CRC32_t *pulCRC, const void *pBuffer, int nBuffer)
{
	union
	{
		...
		{
			CRC32_t ulCrc;
			unsigned char _dontuse00;
		};
		struct
		{
			unsigned char ucCrc0;
			unsigned char ucCrc8;
			unsigned char ucCrc16;
			unsigned char ucCrc24;
			unsigned char _dontuse01;
		};
		struct
		{
			unsigned char _dontuse02;
			CRC32_t ulCrc2;
		};
	};
...
JustAfew:
	...
	{
	case 7:
		ulCrc = pulCRCTable[*pb++ ^ ucCrc0] ^ (ulCrc2);
	...
	case 4:
		ulCrc ^= *(CRC32_t *)pb;
		ulCrc = pulCRCTable[ucCrc0] ^ (ulCrc2);
		ulCrc = pulCRCTable[ucCrc0] ^ (ulCrc2);
		ulCrc = pulCRCTable[ucCrc0] ^ (ulCrc2);
		ulCrc = pulCRCTable[ucCrc0] ^ (ulCrc2);
		*pulCRC = ulCrc;
